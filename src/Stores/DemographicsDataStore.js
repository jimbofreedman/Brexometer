...
class DemographicsDataStore {
  ...
  getQuestionDemographicsData(data) {
    const reqArr = [`question=${data.questionId}`];
    if (data.groupId) {
    ...
    return window.API.get(
      `/api/question_demographics/?${reqArr.join('&')}`
    ).then(response => {
  ...
  getWeightedQuestionAverageDataByAge(question_id) {
    ...
    return window.API.get(
      `/api/question_weighted_demographics/${question_id}/age/`
    )
      .then(response => {
  ...
  getWeightedQuestionAverageDataByGeo(question_id, minAge = 0, maxAge = 200) {
    ...
    return window.API.get(
      `/api/question_weighted_demographics/${question_id}/area/?voter_age__gte=${minAge}&voter_age__lte=${maxAge}`
    )
      .then(response => {
