...
class CollectionStore {
      ...
      this.getCollectionItemsByIdPage(collectionId, 1)
        .then(response => {
          ...
          for (const item of items) {
            if (item.type === 'Q') {
              // If item is a question, update QuestionStore
              window.stores.QuestionStore.questions.set(
                item.content_object.id,
                item.content_object
              ); // Add question to QuestionStore
  ...
  getCollectionItemsByIdPage(collectionId, page) {
    return new Promise((resolve, reject) => {
      window.API.get('/api/question_collection_items/', {
        params: { parent: collectionId, ordering: 'order', page },
      })
        .then(response => {
  ...
  items(collectionId, forceUpdate = false) {
    ...
    window.API.get('/api/question_collection_items/', {
      params: { parent: collectionId, ordering: 'order' },
    })
      .then(response => {
        // Then perform network requests to get questions
        const items = response.data.results;
        for (const item of items) {
          if (item.type === 'Q') {
            // If item is a question, update QuestionStore
  ...
  updateCollection(collectionId, title, description, endText) {
    window.API.patch(`/api/question_collections/${collectionId}/`, {
      // First step is update details
      desc: description,
      end_text: endText,
      name: title,
    })
      .then(response => {
        // Once details updated, check for question changes
