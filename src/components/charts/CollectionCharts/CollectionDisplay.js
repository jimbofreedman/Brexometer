...
const CollectionCharts = props => <CollectionDisplayView {...props} />;

@inject('CollectionStore')
@observer
class CollectionDisplayView extends React.Component {
  constructor(props) {
    super(props);
  }
...
@observer
class CollectionQuestionPieCharts extends React.Component {
  render() {
    ...
    return (
      <div>
        ...
        {this.props.collection.photo && (
          ...
          >
            <img
              src={this.props.collection.photo}
    ...
    return (
      <div style={{ position: 'relative', overflow: 'hidden' }}>
        ...
        <SliderContainer
          ...
          max={items.length - 1 > 1 ? items.length - 1 : 1}
          value={this.state.activeId}
          disabled={items.length - 1 === 0}
        />
  ...
  <div>
    {props.items.map((item, index) => {
      if (index === props.activeId) {
        return (
          <Card
            style={{ padding: 10, margin: 10, marginBottom: 0 }}
            key={index}
          >
            <QuestionLiquidPiechart
              questionId={item.object_id}
              pie={props.pie}
            />
          </Card>
        );
