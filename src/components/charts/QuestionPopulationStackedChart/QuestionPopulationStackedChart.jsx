...
const QuestionPopulationStackedChart = inject(
  'CensusDataStore',
  ...
  ({
    ...
    const certainityStatisticsArr = null;
    const currentlyShowingIndex = null;
    ...
    initData(
      (censusData, demogrData, question) => {
        viewData.values =
          question.choices &&
          generateDemogrChartData(demogrData, question.choices); // generate only if question loaded
...
const QuestionPopulationStackedChartView = observer(({ data, height }) => (
  <div style={{ width: '100%', height: '50%' }}>
    ...
    {data.values && (
      <ResponsiveContainer minWidth={100} minHeight={100} height={height}>
        <AreaChart
          width={600}
          height={400}
          data={data.values}
        ...
        >
          ...
          <YAxis
            orientation="right"
            yAxisId="right"
            tickFormatter={simpleFormatter}
          />
          <CartesianGrid strokeDasharray="3 3" />
          <Tooltip
            labelFormatter={tootltipLabelFormatter}
...
function generateDemogrChartData(demogrData, choices) {
  return choices.length > 0
    ? getDemographicsMcqData(demogrData, choices)
    : getDemographicsLikertData(demogrData);
}

function getDemographicsLikertData(demogrData) {
  const colors = QuestionService.getLikertColors().reverse();
  const demogrValues = demogrData.ageggendervalues;
  const answersNames = [
    'Strongly agree',
    'Agree',
    'Neutral',
  ...
  const SKIP_INDEX = answersNames.length - 1;
  const ages = [
    '<15',
    '15-25',
    '25-35',
  ...
  const ageToIndex = {}; // ageToIndex={"<15":0, "15-25":1, ... }
...
function getDemographicsMcqData(demogrData, choices) {
  const colors = QuestionService.getMcqColors();
  const demogrValues = demogrData.ageggendervalues;

  // to qiuckly get and index of age/answer in resData
  const answerNameToIndex = {};
  const ageToIndex = {};
  const SKIP_INDEX = choices.length;
  ...
  demogrValues.forEach((demogr, i) => {
    ageIndex = ageToIndex[demogr.age_range];
    answerIndex = demogr.object__text
      ? answerNameToIndex[demogr.object__text]
      : SKIP_INDEX;
    resData[ageIndex].values[answerIndex].value += demogr.id__count;
  });
  return resData;
...
function simpleFormatter(val) {
  ...
  const dotPosition = resStr.indexOf('.');
