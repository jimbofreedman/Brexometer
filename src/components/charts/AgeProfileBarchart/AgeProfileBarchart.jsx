...
const AgeProfileBarchart = inject('DemographicsDataStore')(
  ({ DemographicsDataStore, size, geoId }) => {
    const viewData = observable.object(
      {
        width: (size && size.width) || 400,
        height: (size && size.height) || 300,
        values: computeBarchartData(
          DemographicsDataStore.usersDemographicsData.get(geoId)
        ),
      },
      {},
      { deep: false }
    ...
    return <AgeProfileBarchartView data={viewData} />;
  }
);
  ...
  <div style={{ width: data.width, height: data.height }}>
    ...
    {data.values && (
      <BarChart
        ...
        height={data.height}
        data={data.values}
        className="age-profile-barchart"
      >
        <XAxis dataKey="age" />
        <YAxis />
        ...
        <ul>
          {payload.reverse().map(payl => (
            <li className="label" key={payl.name}>
              <span className="color" style={{ backgroundColor: payl.color }} />
...
let computeBarchartData = data => {
  ...
  data.map((value, i) => {
    ...
    */
    ...
    const index = 10 - value.age_group;
    const gender =
      (value.gender == 1 && 'male') ||
      (value.gender == 2 && 'female') ||
      (value.gender == 3 && 'other') ||
      (value.gender == 0 && 'notSay');
