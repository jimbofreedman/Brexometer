...
class UserCard extends Component {
  ...
  componentWillMount() {
    ...
    const location =
      (this.props.user.country_info
        ? this.props.user.country_info.name +
          (this.props.user.region_info ? ', ' : '')
        : '') +
      (this.props.user.region_info ? this.props.user.region_info.name : '');
    const link = `https://app.represent.me/profile/${this.props.user.id}/${
      this.props.user.username
    }`;
  ...
  componentDidMount() {
    if (this.props.compareData) {
      const concensus = `${Math.floor(
        100 - this.props.compareData.difference_percent
      )}%`;
    ...
    switch (e.target.value) {
      case 'disagree': {
        ...
        return this.setState({
          checked: [...checked.slice(0, 3), value, value],
        });
      }
      case 'agree': {
        const checked = this.state.checked;
      ...
      case 'neutral': {
        const checked = this.state.checked;
        return this.setState({
          checked: [...checked.slice(0, 2), value, ...checked.slice(3)],
        });
      }
      default:
    }
  ...
  render() {
    ...
    if (this.props.compareData) {
      ...
      keys.map(k => {
        ...
        // if (arr.length < 5) {
        check1 = arr.map(
          (key, j) => this.props.compareData.topic_diffs[k].diffs[key] > 0
        );
        ...
        if (check1 && check2)
          display = Object.assign(display, {
            [k]: this.props.compareData.topic_diffs[k],
          });
      });
    ...
    return (
      <Card style={{ marginBottom: '20px' }}>
        <CardHeader
          ...
          avatar={this.props.user.photo.replace(
            'localhost:8000',
            'represent.me'
          )}
        />
            ...
            <div>
              <CardText style={{ paddingTop: 0 }}>
                <p style={{ fontSize: 14, fontWeight: 'bold' }}>
                  By how much my answers variate from {name}'s?
                </p>
              </CardText>
              <MatchBarchart compareData={this.props.compareData} />
              <p>{`Compared over: ${
                this.props.compareData.questions_counted
              } questions`}</p>
            </div>
          ) : (
            <LoadingIndicator />
        ...
        {this.props.compareData && (
          <div>
            <CardText style={{ paddingTop: 0 }}>
              <p style={{ fontSize: 14, fontWeight: 'bold' }}>
                Bar height signifies distances in answers:
              </p>
            </CardText>
            <CheckboxComponent
              label="DISTANCE IN DISAGREE"
              fill={labels.strongly_disagree.color}
              handleCheck={this.handleCheck}
              value="disagree"
              checked={this.state.checked[4]}
            />
            <CheckboxComponent
              label="DISTANCE IN AGREE"
              fill={labels.strongly_agree.color}
              handleCheck={this.handleCheck}
              value="agree"
              checked={this.state.checked[0]}
            />
            <CheckboxComponent
              label="IMPORTANT/NEUTRAL"
              fill={labels.neutral.color}
              handleCheck={this.handleCheck}
              value="neutral"
                ...
                .map((d, i) => (
                  ...
                  >
                    <div>
                      ...
                      >
                        <SmallBarchart values_arr={display_all[d].diffs} />{' '}
                        {/* this.props.compareData.topic_diffs[d]['diffs']}/> */}
                      </div>
                      <div
                        style={{
                          display: 'inline-block',
      ...
      >
        {payload.map((p, i) => {
          ...
          return (
            <p
              key={`p-${i}`}
              style={{ color: payload[i].fill, margin: 2 }}
            >{`${name}: ${Math.round(payload[i].payload[name])}%`}</p>
          );
        })}
      </div>
    );
...
const MatchBarchart = observer(({ compareData }) => {
  ...
  return (
    <ResponsiveContainer minHeight={80} maxWidth={150}>
      <BarChart layout="vertical" data={[values]} barGap={1}>
        <XAxis domain={[0, 100]} hide type="number" />
        <YAxis type="category" hide />
        <Bar
          dataKey="strongly_disagree"
          fill={labels[Object.keys(values)[4]].color}
        />
        ...
        <Bar
          dataKey="strongly_agree"
          fill={labels[Object.keys(values)[0]].color}
        />
...
const SmallBarchart = props => {
  ...
  return (
    <ResponsiveContainer minHeight={50} minWidth={50}>
      <BarChart barGap={1} data={[values]}>
        <YAxis domain={[0, 4]} hide type="number" />
