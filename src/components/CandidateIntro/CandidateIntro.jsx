  ...
  render() {
    return (
      <div style={{ display: 'table', width: '100%', height: '100%' }}>
        ...
        <Dialog
          open={this.state.showEmailExistsDialog}
          contentStyle={{ width: '300px' }}
        >
          <div>
            <p style={{ fontWeight: 'bold' }}>
              {
                "It looks like you're already signed up to Represent, please login to continue."
              }
            </p>
            <RaisedButton
              label="Login"
        ...
        >
          <div>
            ...
            <RaisedButton
              label="Continue"
              style={{ width: '100%' }}
              onClick={() => this.setState({ emailInvalid: false })}
            />
          </div>
        </Dialog>
      </div>
  ...
  checkEmail() {
    ...
    this.props.UserStore.checkEmail(this.state.email).then(exists => {
      if (exists) {
        ...
        this.props.history.push(
          `/candidate/new/${encodeURIComponent(this.state.email)}`
        );
      }
    });
  }
